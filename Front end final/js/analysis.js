...
(function () {
  ...
  function initAnalysis() {
    ...
    var dropContent = dropZone
      ? dropZone.querySelector(".relative.z-10")
      : null;
    ...
    console.log(
      "[HelioSense] Rooftop analysis — API base:",
      API_BASE || "(relative — served by Flask)",
    );
    var toast =
      window.showToast ||
      function (msg, t) {
    ...
    function applyResult(data) {
      ...
      var roofM2val =
        data.roof_area_m2 != null ? data.roof_area_m2 : data.total_roof_area;
      var usableM2val =
        data.usable_area_m2 != null ? data.usable_area_m2 : data.usable_area;
      var obsM2val =
        data.obstruction_area_m2 != null ? data.obstruction_area_m2 : null;

      var totalM2 = roofM2val != null ? roofM2val + " m²" : "—";
      var usableM2 = usableM2val != null ? usableM2val + " m²" : "—";
      var pct =
        roofM2val && usableM2val
          ? Math.round((usableM2val / roofM2val) * 100)
          : 0;

      var el;
      el = safeEl("roofAreaValue");
      if (el) el.textContent = totalM2;
      el = safeEl("usableAreaValue");
      if (el) el.textContent = usableM2;
      el = safeEl("roofAreaProgress");
      if (el) el.style.width = "100%";
      el = safeEl("usableAreaProgress");
      if (el) el.style.width = pct + "%";
      setShadeRiskBadge(data.shade_risk);
      el = safeEl("placementValue");
      if (el)
        el.textContent =
          (data.recommended_capacity_kw || "—") +
          " kW · " +
          (data.panel_count || "—") +
          " panels";
      el = safeEl("pitchValue");
      if (el)
        el.textContent = data.confidence != null ? data.confidence + "%" : "—";
      el = safeEl("obstructionsValue");
      if (el) {
        var obsText =
          data.obstruction_count != null
            ? data.obstruction_count + " detected"
            : "—";
      ...
      lastResult = data;
      try {
        localStorage.setItem("rooftopAnalysis", JSON.stringify(data));
      } catch (e) {}
    ...
    function downloadReport() {
      if (!lastResult) {
        toast("No analysis available to download");
        return;
      }
      var roofVal =
        lastResult.roof_area_m2 != null
          ? lastResult.roof_area_m2
          : lastResult.total_roof_area;
      var usableVal =
        lastResult.usable_area_m2 != null
          ? lastResult.usable_area_m2
          : lastResult.usable_area;
      var lines = [
        ...
        "Setback Area:           " +
          (lastResult.setback_area_m2 != null
            ? lastResult.setback_area_m2 + " m²"
            : "—"),
        "Obstruction Area:       " +
          (lastResult.obstruction_area_m2 != null
            ? lastResult.obstruction_area_m2 + " m²"
            : "—"),
        "Suitability Score:      " +
          (lastResult.suitability_score != null
            ? lastResult.suitability_score + "/100"
            : "—"),
        "Confidence:             " +
          (lastResult.confidence != null ? lastResult.confidence + "%" : "—"),
        "Shade Risk:             " + (lastResult.shade_risk || "—"),
        "Obstruction Count:      " +
          (lastResult.obstruction_count != null
            ? lastResult.obstruction_count
            : "—"),
        "Recommended Capacity:   " +
          (lastResult.recommended_capacity_kw || "—") +
          " kW",
    ...
    function handleFile(f) {
      ...
      if (["image/jpeg", "image/png", "image/jpg"].indexOf(f.type) === -1) {
        toast("Please select a JPG or PNG image");
        return;
      }
      selectedFile = f;
    ...
    if (detectBtn) {
      detectBtn.addEventListener("click", function () {
        ...
        detectBtn.innerHTML =
          '<span class="spinner-inline"></span> Detecting...';
        if (!navigator.geolocation) {
          toast("Geolocation not supported");
          detectBtn.disabled = false;
          detectBtn.innerHTML =
            '<span class="material-symbols-outlined">my_location</span> Detect Location';
          return;
        }
        navigator.geolocation.getCurrentPosition(
          function (pos) {
            ...
            if (locationResult)
              locationResult.textContent = "Location: " + lat + ", " + lng;
    ...
    if (analyzeBtn) {
      analyzeBtn.addEventListener("click", function () {
        if (!selectedFile) {
          toast("Please upload a rooftop image first");
          return;
        }
        ...
        fetch(endpoint, { method: "POST", body: form })
              ...
              return resp.text().then(function (body) {
                throw new Error(
                  'Expected JSON but got "' +
                    ct +
                    '". Is the request hitting Flask? Response: ' +
                    body.slice(0, 200),
                );
              });
            }
          ...
          .then(function (data) {
            console.log("[HelioSense] FULL BACKEND RESPONSE:", data);
            console.log(
              "[HelioSense] DEBUG JSON:",
              JSON.stringify(data.debug, null, 2),
            );
            if (!data.success) throw new Error(data.error || "Analysis failed");
            applyResult(data);
            if (fileNameEl)
              fileNameEl.textContent = selectedFile.name + " — Analyzed";
    ...
    function applyZoom() {
      if (heatmapMImg)
        heatmapMImg.style.transform = "scale(" + heatmapZoom + ")";
    }
    ...
    if (zoomIn)
      zoomIn.addEventListener("click", function () {
        heatmapZoom = Math.min(5, heatmapZoom + 0.25);
        applyZoom();
      });
    if (zoomOut)
      zoomOut.addEventListener("click", function () {
        heatmapZoom = Math.max(0.5, heatmapZoom - 0.25);
        applyZoom();
      });
    if (zoomReset)
